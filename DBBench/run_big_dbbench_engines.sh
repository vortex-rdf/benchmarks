#!/usr/bin/env bash
# Hardened DBBench driver for long runs: benchmarks cottas plus the vortex
# engine on the dictionary and default layouts, with an outer watchdog per
# configuration. See README.md for data/query acquisition and prep.
set -euo pipefail
...
usage() {
  cat <<EOF
Usage:
  $0 --query-root PATH --cottas-path PATH --vortex-dictionary-path PATH --vortex-default-path PATH [options]
...
Options:
  ...
  --dataset NAME
  --groups GROUPS
  --timeout-mode MODE               process | signal | worker. Default: ${TIMEOUT_MODE}
  --warmup-runs N
  --measured-runs N
  --max-queries N
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --vortex-dictionary-path)
      VORTEX_DICTIONARY_PATH="$2"; shift 2 ;;
    --vortex-default-path)
      VORTEX_DEFAULT_PATH="$2"; shift 2 ;;
    --warmup-runs)
...
[[ -n "$VORTEX_DICTIONARY_PATH" ]] || die "--vortex-dictionary-path is required"
[[ -n "$VORTEX_DEFAULT_PATH" ]] || die "--vortex-default-path is required"

require_cmd "$PYTHON_BIN"
require_cmd timeout
require_file "$BENCHMARK_SCRIPT"
require_file "$COTTAS_PATH"
require_file "$VORTEX_DICTIONARY_PATH"
require_file "$VORTEX_DEFAULT_PATH"
...

{
  ...
  echo "Vortex dictionary path: $VORTEX_DICTIONARY_PATH"
  echo "Vortex default path: $VORTEX_DEFAULT_PATH"
  ...
  echo "VORTEX_RDF_DICT_MAX_RESIDENT_TERMS: ${VORTEX_RDF_DICT_MAX_RESIDENT_TERMS:-<default>}"
  echo
} | tee "$MANIFEST"
...
run_config "cottas" "cottas" "$VORTEX_DEFAULT_PATH" "default"
run_config "vortex_dictionary" "vortex" "$VORTEX_DICTIONARY_PATH" "dictionary"
run_config "vortex_default" "vortex" "$VORTEX_DEFAULT_PATH" "default"
